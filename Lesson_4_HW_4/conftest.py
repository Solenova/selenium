...
@pytest.fixture()
def user_token():
    result = S.post(url=testdata['url_api'], data={'username': testdata['login'], 'password': testdata['password']})
...
#REST
@pytest.fixture()
def post_create(user_token):
    # Создаем пост на тест-стенде GB
    result = S.post(url=testdata['url_post'], headers={'X-Auth-Token': user_token}, data={
